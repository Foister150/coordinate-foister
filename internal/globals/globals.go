...
func AppendBrokenHost(ip string) {
	stateMu.Lock()
	defer stateMu.Unlock()
	BrokenHosts = append(BrokenHosts, ip)
}

func BrokenHostsSnapshot() []string {
	stateMu.Lock()
	defer stateMu.Unlock()
	return append([]string(nil), BrokenHosts...)
}

func AppendAnnoyingErr(msg string) {
	stateMu.Lock()
	defer stateMu.Unlock()
	AnnoyingErrs = append(AnnoyingErrs, msg)
}

func AnnoyingErrsSnapshot() []string {
	stateMu.Lock()
	defer stateMu.Unlock()
	return append([]string(nil), AnnoyingErrs...)
}

func IncrementTotalRuns() int {
	stateMu.Lock()
	defer stateMu.Unlock()
	TotalRuns++
	return TotalRuns
...
var (
	...
	MaxHosts          = flag.IntP("max-hosts", "m", 0, "Max concurrent host connections (0 = unlimited)")
	...
	PasswordList      = []string{}
	EnvironCmds       = []string{}
	ScriptContentsMap = map[string]string{}
